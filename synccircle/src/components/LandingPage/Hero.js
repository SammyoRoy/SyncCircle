...
const Hero = ({ scrollRef }) => {
    ...
    const customDialogStyle = {
        display: open ? 'block' : 'none', // Show or hide dialog based on 'open' state
        position: 'absolute',
        top: userImageRef.current ? userImageRef.current.offsetTop + userImageRef.current.offsetHeight + 20 : 0,
        left: userImageRef.current ? userImageRef.current.offsetLeft - 120 : 0,
        backgroundColor: '#f5f7fac2',
    ...
    const customBackdropStyle = {
        display: open ? 'block' : 'none', // Show or hide backdrop based on 'open' state
        position: 'fixed',
        top: 0,
        left: 0,
        width: '100%',
    ...
    return (
        <>
            <div className='Top'>
                <Typography variant="h1" display="flex" alignItems="Center" noWrap component="div" marginLeft="10%" sx={{ flexGrow: 1, color: '#6ABEA7', fontFamily: 'Inter', fontWeight: 700, fontSize: '2rem', gap: '20px' }}>
                    ...
                    {open && (
                        <>
                            <div style={customBackdropStyle} onClick={() => setOpen(false)}></div>
